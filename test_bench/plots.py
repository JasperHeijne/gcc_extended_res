...
def extract_learned_clause_length(filepath):
    ...
    """
    learned_clause_length_pattern = (
        r"learnedClauseStatisticsAverageLearnedClauseLength=([0-9]*\.?[0-9]+)"
    )
    error_patterns = ["=====ERROR====="]
    ...
    with open(filepath, "r") as file:
        ...
        return (
            last_learned_clause_length if last_learned_clause_length is not None else 0
        )
...
def extract_conflict_size(filepath):
    ...
    """
    conflict_size_pattern = (
        r"learnedClauseStatisticsAverageConflictSize=([0-9]*\.?[0-9]+)"
    )
    error_patterns = ["=====ERROR====="]
...
def parse_benchmark_dirs(directories, ignore_unkown_status=False):
    ...
    for directory in directories:
        for filename in os.listdir(directory):
            if filename.endswith(".txt") and not filename.startswith("#"):
                ...
                match = re.match(
                    r"(.+?)_(regin|basic_filter|decomp)\.mzn-(.+?)-[0-9]+\.txt",
                    filename,
                )
                if match:
                    ...
                    objective_data[(problem_name, data_file)][technique].append(
                        objective
                    )
                    lbd_data[(problem_name, data_file)][technique].append(lbd)
                    learned_clause_length_data[(problem_name, data_file)][
                        technique
                    ].append(learned_clause_length)
                    conflict_size_data[(problem_name, data_file)][technique].append(
                        conflict_size
                    )
    ...
    avg_runtimes = {
        (problem, data_file): {
            tech: sum(runtimes) / len(runtimes) if runtimes else 0
            for tech, runtimes in techniques.items()
        }
    ...
    standard_deviation_runtimes = {
        (problem, data_file): {
            tech: np.std(runtimes) if runtimes else 0
            for tech, runtimes in techniques.items()
        }
        for (problem, data_file), techniques in benchmark_data.items()
    }
    ...
    avg_lbds = {
        (problem, data_file): {
            tech: sum(lbds) / len(lbds) if lbds else 0
            for tech, lbds in techniques.items()
        }
        for (problem, data_file), techniques in lbd_data.items()
    }
    avg_learned_clause_lengths = {
        (problem, data_file): {
            tech: sum(lengths) / len(lengths) if lengths else 0
            for tech, lengths in techniques.items()
        }
        for (problem, data_file), techniques in learned_clause_length_data.items()
    }
    avg_conflict_sizes = {
        (problem, data_file): {
            tech: sum(sizes) / len(sizes) if sizes else 0
            for tech, sizes in techniques.items()
        }
...
def normalize(avg_values):
    ...
    for (problem, data_file), tech_values in avg_values.items():
        ...
        normalized_values[(problem, data_file)] = {
            tech: (value / decomp_value if decomp_value > 0 else value / MAX_RUNTIME)
            for tech, value in tech_values.items()
        }
...
def plot_benchmarks(
    avg_values,
    abs_values,
    title="Normalized Benchmark Values by Technique",
    hide_problem_name=False,
):
    """
    ...
    abbreviated_problems = [
        f"{str(abbreviate_text(problem, max_length=10)) + ', ' if not hide_problem_name else ''}{abbreviate_text(data_file, max_length=20)}"
        for problem, data_file in problems
    ]
    ...
    for i, technique in enumerate(techniques):
        values = [
            grouped_data[(problem, data_file)][i] for problem, data_file in problems
        ]
        values_abs = [
            grouped_data_abs[(problem, data_file)][i] for problem, data_file in problems
        ]
        bar_positions = [pos + i * bar_width for pos in x]
        plt.bar(
            bar_positions, values, bar_width, label=technique, color=colors[technique]
        )

        # Annotate bars with values
        for pos, value, value_abs in zip(bar_positions, values, values_abs):
            if (
                technique == "regin" or technique == "basic_filter"
            ) and value > 0.000001:
                plt.text(
                    pos,
                    value + 0.15,
    ...
    decomp_positions = [
        pos + bar_width * 0 for pos in x
    ]  # First position corresponds to "decomp"
    plt.plot(
        decomp_positions,
        [1] * len(decomp_positions),
        "-",
        label="Baseline (Decomp = 1)",
        color="black",
        lw=0.5,
    )
...
def plot_all_statistics(
    ...
    markers = {
        ...
        "basic_filter": "s",
        "decomp": "D",
    }  # Different markers for techniques
    ...
    for (
        problem,
        data_file,
    ), tech_learned_clause_lengths in avg_learned_clause_lengths.items():
        for technique in techniques:
            grouped_data_learned_clause_length[(problem, data_file)].append(
                tech_learned_clause_lengths.get(technique, 0)
            )

    for (problem, data_file), tech_conflict_sizes in avg_conflict_sizes.items():
        for technique in techniques:
            grouped_data_conflict_size[(problem, data_file)].append(
                tech_conflict_sizes.get(technique, 0)
            )

    for (problem, data_file), tech_lbds in abs_lbds.items():
        for technique in techniques:
            grouped_data_abs_lbd[(problem, data_file)].append(
                tech_lbds.get(technique, 0)
            )

    for (
        problem,
        data_file,
    ), tech_learned_clause_lengths in abs_learned_clause_lengths.items():
        for technique in techniques:
            grouped_data_abs_learned_clause_length[(problem, data_file)].append(
                tech_learned_clause_lengths.get(technique, 0)
            )

    for (problem, data_file), tech_conflict_sizes in abs_conflict_sizes.items():
        for technique in techniques:
            grouped_data_abs_conflict_size[(problem, data_file)].append(
                tech_conflict_sizes.get(technique, 0)
            )
    ...
    abbreviated_problems = [
        f"{abbreviate_text(problem, max_length=10)}, {abbreviate_text(data_file, max_length=20)}"
        for problem, data_file in problems
    ]
    x = range(len(problems))
    bar_width = 0.2
    ...
    fig, axs = plt.subplots(
        3, 1, figsize=(12, 12), sharex=True, gridspec_kw={"height_ratios": [1, 1, 1]}
    )
    ...
    for i, technique in enumerate(techniques):
        lbds = [
            grouped_data_lbd[(problem, data_file)][i] for problem, data_file in problems
        ]
        lbds_abs = [
            grouped_data_abs_lbd[(problem, data_file)][i]
            for problem, data_file in problems
        ]
        ...
        for pos, lbd, abs_lbd in zip(x_positions, lbds, lbds_abs):
            if (technique == "regin" or technique == "basic_filter") and lbd > 0.01:
                axs[0].text(
                    pos, lbd + 0.1, f"{lbd:.2f}", ha="center", va="bottom", fontsize=6
                )
        ...
        if technique in ["regin", "basic_filter"]:
            axs[0].plot(
                x_positions, lbds, color=colors[technique], linestyle="-", linewidth=1
            )
    ...
    for i, technique in enumerate(techniques):
        learned_clause_lengths = [
            grouped_data_learned_clause_length[(problem, data_file)][i]
            for problem, data_file in problems
        ]
        learned_clause_lengths_abs = [
            grouped_data_abs_learned_clause_length[(problem, data_file)][i]
            for problem, data_file in problems
        ]
        x_positions = [pos + i * bar_width for pos in x]
        axs[1].scatter(
        ...
        for pos, length, abs_length in zip(
            x_positions, learned_clause_lengths, learned_clause_lengths_abs
        ):
            ...
            if technique == "decomp":  # Additional annotation for "decomp"
                axs[1].text(
                    pos, 0, f"{abs_length:.1f}", ha="center", va="top", fontsize=6
                )
    ...
    axs[1].set_title(
        "Normalized Learned Clause Length by Technique (Relative to Decomp)"
    )
    ...
    for i, technique in enumerate(techniques):
        conflict_sizes = [
            grouped_data_conflict_size[(problem, data_file)][i]
            for problem, data_file in problems
        ]
        conflict_sizes_abs = [
            grouped_data_abs_conflict_size[(problem, data_file)][i]
            for problem, data_file in problems
        ]
        x_positions = [pos + i * bar_width for pos in x]
        axs[2].scatter(
        ...
        for pos, size, abs_size in zip(x_positions, conflict_sizes, conflict_sizes_abs):
            if (technique == "regin" or technique == "basic_filter") and size > 0.01:
                axs[2].text(
                    pos, size + 0.1, f"{size:.2f}", ha="center", va="bottom", fontsize=6
                )
            if technique == "decomp":  # Additional annotation for "decomp"
                axs[2].text(
                    pos, 0, f"{abs_size:.1f}", ha="center", va="top", fontsize=6
                )
...
def plot_all_statistics_bar(
    ...
    for (problem, data_file), tech_runtimes in avg_runtimes.items():
        for technique in techniques:
            grouped_data_runtime[(problem, data_file)].append(
                tech_runtimes.get(technique, 0)
            )
    ...
    for (problem, data_file), tech_runtimes in abs_runtimes.items():
        for technique in techniques:
            grouped_data_abs_runtime[(problem, data_file)].append(
                tech_runtimes.get(technique, 0)
            )
    ...
    fig, axs = plt.subplots(
        4, 1, figsize=(12, 16), sharex=True, gridspec_kw={"height_ratios": [1, 1, 1, 1]}
    )

    # Plot runtime bar chart
    for i, technique in enumerate(techniques):
        runtimes = [
            grouped_data_runtime[(problem, data_file)][i]
            for problem, data_file in problems
        ]
        runtimes_abs = [
            grouped_data_abs_runtime[(problem, data_file)][i]
            for problem, data_file in problems
        ]
        bar_positions = [pos + i * bar_width for pos in x]
        axs[0].bar(
            bar_positions, runtimes, bar_width, label=technique, color=colors[technique]
        )
    ...
    for i, technique in enumerate(techniques):
        ...
        bar_positions = [pos + i * bar_width for pos in x]
        axs[1].bar(
            bar_positions, lbds, bar_width, label=technique, color=colors[technique]
        )
    ...
    for i, technique in enumerate(techniques):
        ...
        for pos, length, abs_length in zip(
            bar_positions, learned_clause_lengths, learned_clause_lengths_abs
        ):
            axs[2].text(
                pos,
                length + 0.05,
    ...
    for i, technique in enumerate(techniques):
        ...
        for pos, size, abs_size in zip(
            bar_positions, conflict_sizes, conflict_sizes_abs
        ):
            axs[3].text(
                pos,
                size + 0.05,
...
def plot_runtime_and_objective(
    avg_runtimes, avg_objectives, abs_runtimes, abs_objectives
):
    """
    ...
    for (problem, data_file), tech_objectives in avg_objectives.items():
        for technique in techniques:
            grouped_data_objective[(problem, data_file)].append(
                tech_objectives.get(technique, 0)
            )
    ...
    for (problem, data_file), tech_objectives in abs_objectives.items():
        for technique in techniques:
            grouped_data_abs_objective[(problem, data_file)].append(
                tech_objectives.get(technique, 0)
            )
    ...
    fig, axs = plt.subplots(
        2, 1, figsize=(12, 12), sharex=True, gridspec_kw={"height_ratios": [1, 1]}
    )
    ...
    for i, technique in enumerate(techniques):
        objectives = [
            grouped_data_objective[(problem, data_file)][i]
            for problem, data_file in problems
        ]
        objectives_abs = [
            grouped_data_abs_objective[(problem, data_file)][i]
            for problem, data_file in problems
        ]
        bar_positions = [pos + i * bar_width for pos in x]
        axs[1].bar(
        ...
        for pos, objective, abs_objective in zip(
            bar_positions, objectives, objectives_abs
        ):
...
def print_standard_deviation(avg_runtimes, std):
    ...
    for (problem, data_file), tech_std in std.items():
        ...
        for technique, deviation in tech_std.items():
            ...
            print(
                f"  Technique: {technique}, Average Runtime: {avg_runtime:.2f}, Standard Deviation: {deviation:.2f}"
            )


def print_mean_ratios_and_success_rate(
    avg_runtimes,
    stat_type="Runtime",
    ...
    for tech_runtimes in avg_runtimes.values():
        ...
        if remove_missing and (
            decomp_runtime == 0 or regin_runtime == 0 or basic_filter_runtime == 0
        ):
            continue
        ...
        if not (regin_runtime == 0 and decomp_runtime == 0):
            ...
            regin_ratios.append(
                decomp_runtime / regin_runtime
                if invert
                else regin_runtime / decomp_runtime
            )
        ...
        if not (basic_filter_runtime == 0 and decomp_runtime == 0):
            ...
            basic_filter_ratios.append(
                decomp_runtime / basic_filter_runtime
                if invert
                else basic_filter_runtime / decomp_runtime
            )
    ...
    mean_basic_filter_ratio = (
        np.mean(basic_filter_ratios) if basic_filter_ratios else float("inf")
    )

    geometric_mean_regin_ratio = gmean(regin_ratios) if regin_ratios else float("inf")
    geometric_mean_basic_filter_ratio = (
        gmean(basic_filter_ratios) if basic_filter_ratios else float("inf")
    )

    regin_success_rate = (total_instances - regin_did_not_terminate) / total_instances
    basic_filter_success_rate = (
        total_instances - basic_filter_did_not_terminate
    ) / total_instances
    decomp_success_rate = (total_instances - decomp_did_not_terminate) / total_instances

    print(
        f"Mean {stat_type} Ratio (Regin vs Decomp) {'(invert)' if invert else ''}: arithmetic: {mean_regin_ratio:.2f}, geometric: {geometric_mean_regin_ratio:.2f}"
    )
    print(
        f"Mean {stat_type} Ratio (Basic Filter vs Decomp) {'(invert)' if invert else ''}: arithmetic: {mean_basic_filter_ratio:.2f}, geometric: {geometric_mean_basic_filter_ratio:.2f}"
    )

    if not remove_missing:
        print(
            f"Success Rate (Regin): {regin_success_rate:.2%} (Basic Filter): {basic_filter_success_rate:.2%} (Decomp): {decomp_success_rate:.2%}"
        )
...
plot_runtime_and_objective(
    normalized_runtimes, normalized_objectives, avg_runtimes, avg_objectives
)
...
plot_all_statistics(
    normalized_runtimes,
    normalized_lbds,
    normalized_learned_clause_lengths,
    normalized_conflict_sizes,
    avg_runtimes,
    avg_lbds,
    avg_learned_clause_lengths,
    avg_conflict_sizes,
...
print_mean_ratios_and_success_rate(
    avg_learned_clause_lengths, "LCL", remove_missing=True
)
...
plot_benchmarks(
    normalized_runtimes,
    avg_runtimes,
    title="Normalized Runtimes by Technique",
    hide_problem_name=True,
)
...
print_mean_ratios_and_success_rate(
    avg_runtimes, "Runtimes", invert=True, max_runtime=MAX_RUNTIME / 2
)
print_mean_ratios_and_success_rate(avg_lbds, "LBD", remove_missing=True)
print_mean_ratios_and_success_rate(
    avg_learned_clause_lengths, "LCL", remove_missing=True
...
print_mean_ratios_and_success_rate(
    avg_conflict_sizes, "Conflict Size", remove_missing=True
)
