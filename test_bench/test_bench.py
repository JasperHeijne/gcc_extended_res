...
def print_status(futures, start_time, total_tasks, commands_to_run):
    ...
    print(
        f"Elapsed time: {elapsed_time:.2f} seconds, {completed_tasks}/{total_tasks} tasks finished"
    )

    # Print currently running tasks
    running_tasks = [
        commands_to_run[i][2] for i, future in enumerate(futures) if future.running()
    ]
    if running_tasks:
        print("Currently running tasks:")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process input and output directory paths."
    )
    ...
    with open(inputs, "r") as file:
        ...
        for line in file:
            ...
            if len(parts) == 1:
                input_files.append(
                    (parts[0], "", 1)
                )  # Only one part, second is an empty string
            elif len(parts) == 2:
    ...
    commands = [
        ...
        (
            ...
            f"{pumpkin_command}",
            [
                ("global_cardinality_low_up(", "pumpkin_gcc_regin("),
        ...
        (
            ...
            f"{pumpkin_command}",
            [
                ("global_cardinality_low_up(", "pumpkin_gcc_basic_filter("),
                ("", 'include "pumpkin_gcc.mzn";\n'),
            ],
        ),
    ...
    for future in futures:
        future.add_done_callback(
            lambda f: print_status(futures, start_time, len(futures), commands_to_run)
        )
